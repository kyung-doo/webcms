...
(function( $ ){
	...
	var methods = {
		...
		{
			$(this).data('menuBar').css({display:"block"});
		},

		hideMenuBar : function ()
		{
			$(this).data('menuBar').css({display:"none"});
		},
		...
		init : function( options )
		{
			...
			*/
			...
			{
				menuBar.css({left:editor.offset().left+5, top:editor.offset().top, "width":editor.width()-10});
			});
			$(window).resize();
		...
		createMenuItem: function(itemSettings, options, returnElement){
			...
			if(itemSettings["custom"]){
				...
				menuWrapElement.bind(action, function(e){
		...
		setTextFormat: function(){			
			...
			if($(this).data("commandName") == "fontSize" )
			{
				var item = window.getSelection().focusNode.parentNode;
				$(item).css({"font-size":$(this).data("value")});
			}
